...
os.makedirs(f"results_dmts/seed_{k}/figure", exist_ok=True)
os.makedirs(f"results_dmts/seed_{k}/models", exist_ok=True)
...
for epoch in pbar:
    ...
    if epoch % 1 == 0: 
        torch.save(rnn.state_dict(), f"results_dmts/seed_{k}/models/model_epoch{epoch}.pt")
    ...
    if (epoch % 1000 == 0):
        ...
        plt.subplot(234)
        plt.plot(gt_out[0,:,0].cpu().detach().numpy(),color = cl[0],ls='-')
        plt.plot(gt_out[1,:,0].cpu().detach().numpy(),color = cl[1],ls='-')
        plt.plot(gt_out[2,:,0].cpu().detach().numpy(),color = cl[2],ls='-')
        plt.plot(gt_out[3,:,0].cpu().detach().numpy(),color = cl[3],ls='-')
        plt.plot(pred_out[0,:,0].cpu().detach().numpy(),color = cl[0],ls='--')
        plt.plot(pred_out[1,:,0].cpu().detach().numpy(),color = cl[1],ls='--')
        plt.plot(pred_out[2,:,0].cpu().detach().numpy(),color = cl[2],ls='--')
        plt.plot(pred_out[3,:,0].cpu().detach().numpy(),color = cl[3],ls='--')
        
        plt.subplot(132)
        pred_out, r = rnn(inp)
        r = r.cpu().detach().numpy()
        kappas, dk,kappas_emp,kappa_abs,Nt = get_lpus(rnn,r)
        ...
        plt.scatter(kappas_emp[2,:,0],kappas_emp[2,:,1],1)
        plt.scatter(kappas_emp[3,:,0],kappas_emp[3,:,1],1)
        ...
        plt.contourf(X, Y, Z, levels=[-1, 0, 1], colors=["red","green"], alpha=0.3)
        ...
        plt.quiver(
            ...
            Us_norm, Vs_norm,
            angles="xy",
            scale=30,       # adjust to control arrow size
            width=0.003,
            color="black"
        )
        plt.imshow(
        ...
        plt.scatter(kappas_emp[0,:,0],kappas_emp[0,:,1],10)
        plt.scatter(kappas_emp[1,:,0],kappas_emp[1,:,1],10)
        plt.scatter(kappas_emp[2,:,0],kappas_emp[2,:,1],10)
        plt.scatter(kappas_emp[3,:,0],kappas_emp[3,:,1],10)
        ...
        #plt.xlim([-3,3])
        #plt.ylim([-5,-2])
        plt.savefig(f"results_dmts/seed_{k}/figure/plot_epoch{epoch}.pdf")        
        plt.show()
...
np.savez(f'results_dmts/seed_{k}/final.npz',loss_history=loss_history,grad_history=grad_history,accuracy_history=accuracy_history)


torch.save(rnn.state_dict(), f"results_dmts/seed_{k}/model_final.pt")
...
plt.semilogy((grad_history),color = colors[1],alpha = 0.1)
plt.subplot(234)
plt.plot(gt_out[0,:,0].detach().numpy(),color = cl[0],ls='-')
plt.plot(gt_out[1,:,0].detach().numpy(),color = cl[1],ls='-')
plt.plot(gt_out[2,:,0].detach().numpy(),color = cl[2],ls='-')
plt.plot(gt_out[3,:,0].detach().numpy(),color = cl[3],ls='-')
plt.plot(pred_out[0,:,0].detach().numpy(),color = cl[0],ls='--')
plt.plot(pred_out[1,:,0].detach().numpy(),color = cl[1],ls='--')
plt.plot(pred_out[2,:,0].detach().numpy(),color = cl[2],ls='--')
plt.plot(pred_out[3,:,0].detach().numpy(),color = cl[3],ls='--')
...
plt.scatter(x_coords, y_coords, marker="x",
            color = 'purple', s=20)
plt.contourf(X, Y, Z, levels=[-1, 0, 1], colors=["red","green"], alpha=0.3)
plt.savefig(f"results_dmts/seed_{k}/final.pdf")        
plt.show()

...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep  3 15:05:03 2025
...
class RNN(nn.Module):
    def __init__(self,
        ...
        if sd is not None:
            nn.init.normal_(self.W_rec,mean = 0,std = sd/np.sqrt(hid_dim))
        else:
...
save_epochs = [100,1000,50000,num_epochs-1]
saved_r = {}
saved_acc = {}
state_dicts ={}
out_dicts ={}
...
for epoch in tqdm(range(num_epochs)):
    ...
    if epoch % 1000 == 0:
        ...
        plt.semilogy(grad_history,alpha=0.5)
...
plt.semilogy(range(1,len(loss_history)+1)[::10],loss_history[::10])
plt.semilogy(range(1,len(loss_history)+1)[::10],grad_history[::10],alpha = 0.5)
